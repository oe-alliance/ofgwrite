...
int ubiattach_main(int argc, char * const argv[])
{
	int err;
	libubi_t libubi;
	...
	if (err < 0) {
		if (args.dev)
			my_printf("cannot attach \"%s\"", args.dev);
		else
			my_printf("cannot attach mtd%d", args.mtdn);
		goto out_libubi;
	} else if (err == 1) {
		/* The kernel did not support the 'max_beb_per1024' parameter */
		my_printf("the --max-beb-per1024=%d parameter was ignored", args.max_beb_per1024);
		my_printf("the UBI kernel driver does not support does not allow changing the reserved PEBs count");
		my_printf("the support was added in kernel version 3.7, probably you are running older kernel?");
		goto out_libubi;
	}
	...
	my_printf("UBI device number %d, total %d LEBs (", dev_info.dev_num, dev_info.total_lebs);
	ubiutils_print_bytes(dev_info.total_bytes, 0);
	my_printf("), available %d LEBs (", dev_info.avail_lebs);
